...
@ccclass('Gem/ListBox')
@menu('Gem/ListBox')
export class ListBox extends Gem {
  ...
  private getContentNode(): Node {
    if (!this.content) {
      throw new FastError(`[VirtualScrollView] :${this.node.name} 请在属性面板绑定 content 容器节点`);
    }
    return this.content!;
  }
  ...
  private getViewportMainSize(): number {
    return this.isVertical() ? this._viewportTf.height : this._viewportTf.width;
  }

  private getContentMainPos(): number {
    return this.isVertical() ? this.content!.position.y : this.content!.position.x;
  }

  private setContentMainPos(pos: number) {
  ...
  protected async didLaunch() {
    ...
    if (this.useDynamicSize) await this.initDynamicSizeMode();
    else await this.initFixedSizeMode();
    this.bindTouch();
    this.bindGlobalTouch();
  }
  ...
  private async initFixedSizeMode() {
  ...
  private async initDynamicSizeMode() {
    if (this.getItemHeightFn) {
      this.logger.d('[VirtualScrollView] 使用外部提供的 getItemHeightFn');
      this._itemSizes = [];
      ...
      if (this.itemCreationMode === ItemCreationMode.NODE && this.itemNodes.length > 0) {
        this.logger.d('[VirtualScrollView] 初始化节点池（Node 模式）');
      ...
      // Prefab 模式
        ...
        this.logger.d('[VirtualScrollView] 初始化节点池（Prefab 模式）');
  ...
  private buildPrefixSum() {
    ...
    if (this.isVertical()) this._contentTf.height = Math.max(this._contentSize, this._viewportSize);
  ...
  private calcVisibleRange(scrollPos: number): { start: number; end: number } {
    ...
    const start = this.posToFirstIndex(scrollPos);
  ...
  protected didTick(dt: number) {
    ...
    if (this._isRefreshing && this._refreshState === RefreshState.REFRESHING) {
      // 刷新中，保持在刷新位置
      const refreshPos = this.isVertical() ? -this.pullRefreshThreshold : this.pullRefreshThreshold;
      ...
      if (this.disableBounce) {
        this.setContentMainPos(minBound);
        this._velocity = 0;
        return;
      }
      ...
      // 如果禁用越界滚动，直接限制位置并停止速度
      if (this.disableBounce) {
        this.setContentMainPos(maxBound);
        this._velocity = 0;
        return;
      }
    ...
    if (this._velocity !== 0) {
      ...
      if (this.pixelAlign) pos = Math.round(pos);
      this.setContentMainPos(pos);
      if (this.useVirtualList) this.updateVisible(false);
    }
  }
  ...
  public updateItemHeight(index: number, newSize?: number) {
    ...
    this.rebuildPrefixSumFrom(index);
  ...
  public setTotalCount(count: number) {
    ...
    if (this.useDynamicSize) {
      ...
      this.buildPrefixSum();
      if (this.totalCount > oldCount) this.expandSlotsIfNeeded();
    } else {
      this.recomputeContentSize();
    }
  ...
  private scrollToPosition(targetPos: number, animate = false, duration?: number) {
    ...
    if (!animate) {
      this.setContentMainPos(this.pixelAlign ? Math.round(targetPos) : targetPos);
      this.updateVisible(true);
    } else {
      const currentPos = this.getContentMainPos();
      const distance = Math.abs(targetPos - currentPos);
      // 如果提供了 duration 则使用，否则根据距离自动计算
      const finalDuration = duration !== undefined ? duration : Math.max(0.2, distance / 3000);
      const targetVec = this.isVertical() ? new Vec3(0, targetPos, 0) : new Vec3(targetPos, 0, 0);
  ...
  private onOffSortLayerOperation() {
    for (const element of this._slotNodes) {
      const sitem = element?.getComponent(ListItem);
      if (sitem) {
  ...
  private flashToPosition(targetPos: number) {
    targetPos = math.clamp(targetPos, this._boundsMin, this._boundsMax);
    if (this._scrollTween) {
      this._scrollTween.stop();
      this._scrollTween = null;
  ...
  private stopTouchEvent(e?: EventTouch) {
    if (!e) return;
  ...
  private onDown(e: EventTouch) {
    ...
    if (this.enablePageSnap) {
      this._pageStartPos = this.getContentMainPos();
    }

    this.stopTouchEvent(e);
  ...
  private onMove(e: EventTouch) {
    ...
    if (!this._hasDeterminedScrollDirection) {
      ...
      if (totalDelta > this._scrollDirectionThreshold) {
        ...
        const isListVertical = this.isVertical();
        ...
        if ((isListVertical && isVerticalScroll) || (!isListVertical && isHorizontalScroll)) {
          ...
          const minBound = Math.min(this._boundsMin, this._boundsMax);
          const maxBound = Math.max(this._boundsMin, this._boundsMax);
          const delta = this.isVertical() ? uiDelta.y : uiDelta.x;

          // 判断滑动方向
          const scrollingToStart = this.isVertical() ? delta > 0 : delta < 0;
          const scrollingToEnd = this.isVertical() ? delta < 0 : delta > 0;

          // 只有在非边界位置，或者在边界但向内滑动时才拦截
          const atStartBound = this.isVertical() ? pos <= minBound : pos >= maxBound;
          const atEndBound = this.isVertical() ? pos >= maxBound : pos <= minBound;
    ...
    if (this.enablePullRefresh && !this._isRefreshing) {
      ...
      const atTopBound = this.isVertical() ? pos <= minBound : pos >= maxBound;
      const pullingDown = this.isVertical() ? delta < 0 : delta > 0;

      if (atTopBound && pullingDown) {
        isPullingRefresh = true;
        const overOffset = this.isVertical() ? minBound - pos : pos - maxBound;
        ...
        if (this._pullOffset >= this.pullRefreshThreshold) {
          this.updateRefreshState(RefreshState.READY, this._pullOffset);
        } else {
          this.updateRefreshState(RefreshState.PULLING, this._pullOffset);
        }
      }
    }
    ...
    if (this.enableLoadMore && !this._isLoadingMore && this._hasMore) {
      ...
      const atBottomBound = this.isVertical() ? pos >= maxBound : pos <= minBound;
      const pullingUp = this.isVertical() ? delta > 0 : delta < 0;

      if (atBottomBound && pullingUp) {
        isPullingLoadMore = true;
        const overOffset = this.isVertical() ? pos - maxBound : minBound - pos;
        ...
        if (this._loadOffset >= this.loadMoreThreshold) {
          ...
          this.updateLoadMoreState(LoadMoreState.PULLING, this._loadOffset);
        }
      }
    }
  ...
  private onUp(e?: EventTouch) {
    ...
    if (this._refreshState !== RefreshState.REFRESHING) {
      this._pullOffset = 0;
      this.updateRefreshState(RefreshState.IDLE, 0);
    }
    ...
    // 分页吸附模式：根据滑动距离判断翻页
    if (this.enablePageSnap) {
      this.performPageSnapByDistance();
    }
  }

  // 更新刷新状态
  private updateRefreshState(state: RefreshState, offset: number) {
  ...
  // 更新加载状态
  private updateLoadMoreState(state: LoadMoreState, offset: number) {
  ...
  private triggerRefresh() {
    ...
    this._velocity = 0;
    this.updateRefreshState(RefreshState.REFRESHING, this.pullRefreshThreshold);
  }
  ...
  private triggerLoadMore() {
    ...
    this._velocity = 0;
    this.updateLoadMoreState(LoadMoreState.LOADING, this.loadMoreThreshold);
  }
  ...
  public finishRefresh(success: boolean = true) {
    ...
    this._pullOffset = 0;
    this.updateRefreshState(success ? RefreshState.COMPLETE : RefreshState.IDLE, 0);
  ...
  public finishLoadMore(hasMore: boolean = true) {
    ...
    if (!hasMore) {
      this.updateLoadMoreState(LoadMoreState.NO_MORE, 0);
    } else {
      this.updateLoadMoreState(LoadMoreState.COMPLETE, 0);
      // 延迟重置到 IDLE 状态
      this.scheduleOnce(() => {
  ...
  private updateVisible(force: boolean) {
    ...
    let newFirst = 0;
    if (this.useDynamicSize) {
      const range = this.calcVisibleRange(searchPos);
    ...
    if (diff > 0) {
      ...
      for (let i = 0; i < absDiff; i++) {
        ...
        if (idx >= this.totalCount) {
          ...
          this.layoutSingleSlot(this._slotNodes[slot], idx, slot);
        }
      }
    } else {
      ...
      for (let i = 0; i < absDiff; i++) {
        ...
        if (idx >= this.totalCount) {
          ...
          this.layoutSingleSlot(this._slotNodes[i], idx, i);
        }
      }
    }
  }
  ...
  private async layoutSingleSlot(node: Node | null, idx: number, slot: number) {
    ...
    if (this.useDynamicSize) {
      ...
      newNode.active = true;
      this.updateItemClickHandler(newNode, idx);
      ...
      if (this.getItemHeightFn) {
        ...
        const actualSize = this.isVertical() ? uit?.height || 100 : uit?.width || 100;
      ...
      if (this._needAnimateIndices.has(idx)) {
        ...
        else this.playDefaultItemAppearAnimation(newNode, idx);
      ...
      this.updateItemClickHandler(node, idx);
      if (this.renderItemFn) this.renderItemFn(node, idx);
      if (this._needAnimateIndices.has(idx)) {
        if (this.playItemAppearAnimationFn) this.playItemAppearAnimationFn(node, idx);
        else this.playDefaultItemAppearAnimation(node, idx);
        this._needAnimateIndices.delete(idx);
      }
    }
  }

  private playDefaultItemAppearAnimation(node: Node, index: number) {}
  ...
  private layoutSlots(firstIndex: number, forceRender: boolean) {
    ...
    for (let s = 0; s < this._slots; s++) {
      ...
      if (idx >= this.totalCount) {
        ...
        this.layoutSingleSlot(node, idx, s);
      }
    }
  }
  ...
  public scrollToPage(pageIndex: number, animate: boolean = true) {
    ...
    const targetPos = this.getPagePosition(pageIndex);
    this.scrollToPosition(targetPos, animate, this.pageSnapDuration);
  ...
  private getNearestPageIndex(): number {
    ...
    if (this.useDynamicSize) {
      ...
      return math.clamp(line, 0, this.getMaxPageIndex());
    }
  }
  ...
  private performPageSnap() {
    ...
    const nearestPage = this.getNearestPageIndex();
    const targetPage = math.clamp(nearestPage, 0, this.getMaxPageIndex());

    const targetPos = this.getPagePosition(targetPage);
    const currentPos = this.getContentMainPos();
    ...
    this._velocity = 0;
    this.scrollToPosition(targetPos, true, this.pageSnapDuration);
  ...
  private getPageIndexByPosition(pos: number): number {
    ...
    if (this.useDynamicSize) {
      return this.posToFirstIndex(searchPos);
    } else {
      const stride = this.itemMainSize + this.spacing;
      const adjustedPos = Math.max(0, searchPos - this.headerSpacing);
